  ...
  data() {
    return {
      ...
      users: [
        ...
        { id: 3, username: 'user1', name: '普通用户1', email: 'user1@example.com', status: 0 }
      ],
      roles: [
      ...
      permissions: [
        ...
        { id: 4, name: '权限管理', code: 'permission:view', type: 'menu', resourcePath: '/system/permission' },
        { id: 5, name: '租户管理', code: 'tenant:view', type: 'menu', resourcePath: '/system/tenant' }
      ],
      tenants: [
    ...
    handleLogin() {
      ...
      setTimeout(() => {
        if (this.loginForm.tenantCode && this.loginForm.username) {
          ...
          this.userInfo = { 
            name: this.loginForm.username === 'admin' ? '系统管理员' : (this.loginForm.username === 'test' ? '测试用户' : this.loginForm.username),
            username: this.loginForm.username,
            id: this.loginForm.username === 'admin' ? 1 : 2,
            email: this.loginForm.username + '@example.com'
          }
          this.tenantInfo = {
            name: this.loginForm.tenantCode === 'system' ? '系统管理租户' : '租户-' + this.loginForm.tenantCode,
            code: this.loginForm.tenantCode,
            id: this.loginForm.tenantCode === 'system' ? 1 : 2
          }
